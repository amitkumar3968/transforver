...
static void parse_args(int argc, char *argv[])
{
  ...
  float vocode_volume = 1.0;
  int vocode_band_count = 16;

  if (argc <= 1) 
    {
      char buf[16];
      char *vocode_modulator_filename = error_malloc(PATH_MAX);
      char *vocode_carrier_filename = error_malloc(PATH_MAX);
      char *vocode_output_filename = error_malloc(PATH_MAX);
      ...
      if (buf[0])  
        {
          int vocode_window_length = atoi(buf);
      ...
      if (buf[0])  
        {
          int vocode_window_overlap = atoi(buf);
      ...
      argv[6][0] != '-')
    {
      char *vocode_modulator_filename = argv[1];
      char *vocode_carrier_filename = argv[2];
      char *vocode_window_length = atoi(argv[3]);
      got_window_length = TRUE;
	  int vocode_window_overlap = atoi(argv[4]);
      got_window_overlap = TRUE;
      vocode_band_count = atoi(argv[5]);
      char *vocode_output_filename = argv[6];
    }
  else
    {
      for (i = 1; i < argc && argv[i][0] == '-'; ++i)
        {
          if (argv[i][1] == 'v' && i < argc - 1)
            ...
            int vocode_window_overlap = atoi(argv[++i]);
            got_window_overlap = TRUE;
          }
          else
            usage();
      ...
      char *vocode_modulator_filename = argv[i++];
      char *vocode_carrier_filename = argv[i++];
      char *vocode_output_filename = argv[i++];
    }
}
